        ...
        if (isset($_POST["req"])) {
            switch ($_POST["req"]) {
                case "event":
                    ...
                    $event_title = htmlspecialchars(trim($_POST['title']));
                    $event_date = htmlspecialchars(trim($_POST['event_date']));
                    $event_details = htmlspecialchars(trim($_POST['details']));
                        ...
                        $photo_type = $photo['type'];
                         ...
                         if ($photo_error === UPLOAD_ERR_OK) {
                            ...
                            if (move_uploaded_file($photo_tmp_name, $upload_file_path)) {
                                ...
                                $result = $create->addEvent($event_title, $event_date, $event_details, $photo_url);
                                
                                // Respond with success
                                $body = ["result" => $result];
                                header("Location: " . 'http://localhost/Assignment/server-side/event.php');
